...
**The 3 starter posts** (SEO-optimised, ~700-900 words each, written to match the brand's existing tone) are seeded via `api/seed-blog-posts.ts` (checks slugs first, safe to run more than once) — trigger it from the empty state in the admin Blog section ("Publish 3 Starter Articles"), rather than me inserting them directly (no way to run SQL against the live database from here):
