...
**Public pages**: `/blog` (index, card grid of published posts — no publish date shown on cards or post pages, by design) and `/blog/:slug` (individual post, Markdown content rendered via the `marked` package, `BlogPosting` JSON-LD for SEO — the JSON-LD still includes `datePublished` even though it's not shown on-page, since that's a genuine SEO signal search engines use, distinct from user-visible UI). Both fetch from the public, read-only `api/blog-posts.ts` endpoint, which only ever returns `status = 'published'` posts.

**Admin panel** (`/admin` → **Blog** in the sidebar, `src/pages/admin/BlogAdmin.tsx`): list all posts (including drafts) with status badges, write/edit in a Markdown textarea (`## heading`, `**bold**`, `[link text](/url)`, `- bullet`), optionally set a **Related Service** (a dropdown of all 20 services) which does two things automatically: the service page shows a "From Our Blog" card linking to that post, and the post page shows a "Related Service" card linking back — a genuine two-way link, not just one direction. Save as draft or publish immediately, delete with confirmation. Backed by `api/admin-blog-posts.ts` (auth-protected CRUD) — same Postgres `blog_posts` table the public endpoint reads from.

**24 starter posts total**, seeded via `api/seed-blog-posts.ts` (checks slugs first, safe to run repeatedly) — trigger via **"Publish Starter Articles"** in the admin Blog toolbar (always visible, so re-running it after an earlier batch picks up anything new):
- **4 general/regulatory posts** (not tied to one specific service): the emergency plumber guide, and 3 posts grounded in genuine web research on live 2026 UK housing regulation (Awaab's Law, MEES 2030 energy efficiency changes, the Renters' Rights Act) — see git history for the full list and what each links to.
- **20 service-specific posts, one per service, each linked via Related Service** — covering every single service page, e.g. *What Counts as a Responsive Repair?*, *Common Plumbing Problems in Older London Properties*, *Damp & Mould Treatment: What Actually Works*, *Commercial Refurbishment: Minimising Disruption While You're Still Trading*, and so on for all 20. Two of these reuse existing posts that already matched a service well (planned maintenance, bathroom refurbishment) rather than duplicating content; the other 18 are new, each ~350-500 words — intentionally more tightly scoped than the longer regulatory posts, since the goal here is one genuinely useful, specific article per service rather than padding every single one to the same length regardless of what the topic actually needs.

All internal links use genuine `<a href>` markdown links (crawlable) with descriptive, varied anchor text.
...
**Footer**: "Blog" link in the Company column.
