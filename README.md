...
**Public pages**: `/blog` (index, card grid of published posts) and `/blog/:slug` (individual post, Markdown content rendered via the `marked` package, `BlogPosting` JSON-LD for SEO). Both fetch from the public, read-only `api/blog-posts.ts` endpoint, which only ever returns `status = 'published'` posts.

**Admin panel** (`/admin` → **Blog** in the sidebar, `src/pages/admin/BlogAdmin.tsx`): list all posts (including drafts) with status badges, write/edit in a Markdown textarea (`## heading`, `**bold**`, `[link text](/url)`, `- bullet`), save as draft or publish immediately, delete with confirmation. Backed by `api/admin-blog-posts.ts` (auth-protected CRUD) — same Postgres `blog_posts` table the public endpoint reads from, so there's no separate "publish" step beyond setting status to `published`; it's live the moment you save.

**6 starter posts total** (SEO-optimised, ~700-1000 words each, written to match the brand's existing tone) are seeded via `api/seed-blog-posts.ts` (checks slugs first, safe to run repeatedly — only inserts posts that don't already exist) — trigger it via **"Publish Starter Articles"** in the admin Blog toolbar (always visible, not just on an empty list, so re-running it after the first batch picks up any new ones added later), rather than me inserting them directly (no way to run SQL against the live database from here). The first 3 are general service topics; the next 3 (added later, in the same file/mechanism) are grounded in genuine web research on live 2026 UK housing regulation, since that's both genuinely useful to the target audience and a topic most competitor sites haven't caught up with yet:
- *5 Signs You Need an Emergency Plumber in London (And When It Can Wait)* — links to `/emergency`, `/services/leak-detection`, `/services/drain-unblocking`, `/report-repair`, `/coverage`, `/quote`
- *Why Planned Maintenance Contracts Save Housing Associations Money in the Long Run* — links to `/services/planned-maintenance`, `/services/void-refurbishment`, `/sectors/housing-associations`, `/sectors/local-authorities`, `/contact`, `/about`
- *How to Plan a Bathroom Refurbishment: A Step-by-Step Guide for London Homeowners* — links to `/services/bathroom-refurbishment`, `/services/bathroom-installation`, `/services/tiling`, `/quote`, `/projects`
- *Awaab's Law: What Housing Associations and Local Authorities Need to Do Now* — the actual current legal timeframes (24hrs for emergency hazards; 10 working days to investigate damp/mould, 3 days to report findings, 5 days to act), confirmed 2026/2027 scope expansion — links to `/services/planned-maintenance`, `/sectors/housing-associations`, `/sectors/local-authorities`, `/contact`
- *MEES 2030: What the New Energy Efficiency Rules Mean for London Landlords* — the January 2026 government confirmation of EPC C by 1 October 2030 (single date, not phased as originally proposed), the new dual-metric standard, the £30,000 max fine — links to `/services/planned-maintenance`, `/sectors/landlords`, `/quote`, `/contact`
- *The Renters' Rights Act Is Now in Force: What Every London Landlord Needs to Know* — Section 21 abolition (took effect 1 May 2026), the shift to Assured Periodic Tenancies, Section 13 rent-increase process — links to `/sectors/landlords`, `/quote`, `/contact`, and to the MEES post above (a blog-to-blog internal link, reinforcing the topic cluster)

All internal links use genuine `<a href>` markdown links (crawlable) with descriptive, varied anchor text — same principles as the earlier internal-linking pass.

**Known limitation, flagged honestly**: individual blog post URLs aren't in the static `public/sitemap.xml` — that file is generated by hand for content known at build time (services, sectors, locations), but blog posts are now dynamic, database-driven content that can be added anytime from the admin panel. The blog *index* (`/blog`) is in the sitemap and links to every published post, so posts are still discoverable and crawlable — they just won't appear as their own sitemap entries. A proper fix would be a dynamically-generated sitemap endpoint (query published posts, output XML on request) — a reasonable next step if it matters, not built here to keep this already-large change reviewable.

**Footer**: new "Blog" link in the Company column.

## Prefooter trust bar

Researched `ninjaplumbers.co.uk` (a business Tamesis has a relationship with) for the *type* of trust credentials commonly shown in a prefooter band on UK trade sites — certification/registration, review count + rating, years in business, insurance, coverage breadth. Built an equivalent band for Tamesis (`Footer.tsx`, sits above the "Brand statement" section, so it's sitewide) using Tamesis's own already-established real facts, reworded rather than copied: directly employed engineers (Tamesis's actual differentiator, standing in for the certification slot), 4.6★ from 520+ reviews, trusted since 2014, fully insured & compliant, London-wide coverage.
