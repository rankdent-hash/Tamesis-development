...
export function BlogAdmin() {
  ...
  const [seeding8, setSeeding8] = useState(false);
  ...
  const handleSeedBatch3 = async () => {
    setSeeding3(true);
    setSeedMessage(null);
    const result = await seedBlogPostsBatch3();
    setSeeding3(false);
    if (result.success) {
      setSeedMessage(
        result.inserted && result.inserted > 0
          ? `Added ${result.inserted} new IKEA & flat-pack article${result.inserted === 1 ? "" : "s"}.`
          : "Everything from this batch is already published."
      );
      load();
    } else {
  ...
  const handleSeedBatch6 = async () => {
    setSeeding6(true);
    setSeedMessage(null);
    const result = await seedBlogPostsBatch6();
    setSeeding6(false);
    if (result.success) {
      setSeedMessage(
        result.inserted && result.inserted > 0
          ? "Added the condensation guide."
          : "This post is already published."
      );
      load();
    } else {
  ...
  const handleSeedBatch8 = async () => {
    setSeeding8(true);
    setSeedMessage(null);
    const result = await seedBlogPostsBatch8();
    setSeeding8(false);
    if (result.success) {
      setSeedMessage(
        result.inserted && result.inserted > 0
          ? "Added the handyman services catalog article."
          : "This post is already published."
      );
      load();
    } else {
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={handleSeedBatch8} disabled={seeding8}>
            <Sparkles size={13} /> {seeding8 ? "Checking..." : "Publish Handyman Services Catalog (1 post)"}
          </Button>
