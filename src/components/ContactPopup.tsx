...
interface ContactPopupProps {
  ...
  // When provided, the popup is positioned as a small anchored popover next
  // to the trigger element (top of popup aligned with the icon, popup's
  // top-right corner sitting at the icon's left edge) instead of the
  // default centered full-screen modal.
...
export function ContactPopup({ open, onClose, mode, label, phoneNumber, anchorRect }: ContactPopupProps) {
  ...
  const content = (
    <div
      ...
      className={
        anchorRect
          ? "relative w-72 rounded-2xl bg-white shadow-card-hover p-5 text-center animate-in fade-in zoom-in-95 duration-150 origin-top-right"
          : "relative w-full max-w-xs rounded-2xl bg-white shadow-card-hover p-6 text-center animate-in fade-in zoom-in-95 duration-150"
      }
    >
      <button
        onClick={onClose}
  ...
  if (anchorRect) {
    // Anchored popover: top of popup aligned with the icon's top, popup's
    // top-right corner sitting at the icon's left edge (so it opens
    // leftward from the icon rather than covering it). Fixed positioning
    // since anchorRect is in viewport coordinates. No backdrop dimming —
    // just an invisible click-catcher to close on outside click.
    const gap = 10;
    const style: CSSProperties = {
      top: Math.max(8, anchorRect.top),
      right: Math.max(8, window.innerWidth - anchorRect.left + gap),
    };
    return (
      <div className="fixed inset-0 z-[100]" onClick={onClose} role="dialog" aria-modal="true" aria-label={`Contact ${label}`}>
